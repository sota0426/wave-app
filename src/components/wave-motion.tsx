...
function AnimatedWaveLine({
  ...
  useFrame(() => {
    ...
    for (let i = 0; i <= numPoints; i++) {
      ...
      const y = amplitude * Math.sin(distance * frequency - timeRef.current * speed + phaseShift)
      array[i * 3 + 1] = y
    }
...
useFrame((state, delta) => {
  ...
  // 黄色の玉の位置を常に更新
  if (selectedPoint) {
    const selectedPos = new THREE.Vector3(...selectedPoint);
    ...
    <>
      ...
      { selectedPoint && (
        <>
          ...
          <AnimatedWaveLine
            start={source1Pos}
            end={new THREE.Vector3(...selectedPoint)}
            color="red"
            phaseShift={0}
      ...
      {selectedPoint && (
          <mesh
            ref={yellowSphereRef}
            position={new THREE.Vector3(...selectedPoint)}
