...
export default function WaveInterferenceCircularMotion() {
  ...
  const [speed, setSpeed] = useState(0.0000001)
  ...
  useAnimationFrame(() => {
    if (isRunning) {
      setTime((prevTime) => prevTime + 0.0004 * speed) // Approximately 60 FPS
    }
  })
  ...
  const getWavePoints = (freq: number, phase: number) => {
    return Array.from({ length: numPoints }, (_, i) => {
      ...
      const y = amplitude * Math.sin(2 * Math.PI * freq * (x / waveWidth / scale / 100 - time) + phase * Math.PI / 180)
      return { x, y }
    })
  }

  const getCirclePoint = (freq: number, phase: number) => {
    const angle = 2 * Math.PI * freq * time + phase * Math.PI / 180
    return {
  ...
  const renderWave = (points: { x: number, y: number }[], color: string) => (
    <path
      d={`M ${points.map(p => `${p.x + waveWidth/2},${p.y + waveHeight/2}`).join(' L ')}`}
      fill="none"
      stroke={color}
      strokeWidth="2"
    />
  ...
  const renderCircle = (point: { x: number, y: number }, color: string) => (
    <motion.circle
      cx={circleWidth/2}
      cy={circleHeight/2}
      r="4"
  ...
  const renderWavePoint = (point: { x: number, y: number }, color: string) => (
    <motion.circle
      cx={waveWidth/2}
      cy={waveHeight/2}
      r="4"
      ...
      <div className="flex flex-col lg:flex-row mb-8 space-y-4 lg:space-y-0 lg:space-x-4">
        <div className="w-full lg:w-1/4 h-[300px] relative border border-gray-300 rounded">
          <svg width={circleWidth} height={circleHeight} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            ...
            {renderCircle(circle1Point, getColor(0))}
            {renderCircle(circle2Point, getColor(1))}
            {renderCircle(combinedCirclePoint, getColor(2))}
        ...
        <div className="w-full lg:w-3/4 h-[300px] relative border border-gray-300 rounded">
          <svg width={waveWidth} height={waveHeight} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            ...
            {renderWave(wave1Points, getColor(0))}
            {renderWave(wave2Points, getColor(1))}
            {renderWave(combinedWavePoints, getColor(2))}
            <line x1={0} y1={waveHeight/2 + combinedCirclePoint.y * (waveHeight/2/circleRadius)} x2={waveWidth} y2={waveHeight/2 + combinedCirclePoint.y * (waveHeight/2/circleRadius)} stroke="rgba(0,0,0,0.3)" strokeWidth="1" strokeDasharray="5,5" />
            {renderWavePoint({ x: wave1Points[Math.floor(numPoints/2)].x, y: circle1Point.y * (waveHeight/5/circleRadius) }, getColor(0))}
            {renderWavePoint({ x: wave2Points[Math.floor(numPoints/2)].x, y: circle2Point.y * (waveHeight/5/circleRadius) }, getColor(1))}
            {renderWavePoint({ x: combinedWavePoints[Math.floor(numPoints/2)].x, y: combinedCirclePoint.y * (waveHeight/5/circleRadius) }, getColor(2))}
          </svg>
        </div>
      </div>
